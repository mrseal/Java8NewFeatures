...
public class Main {
    ...
    public static void main(final String[] args) {
        ...
        final Consumer<Person> call = p -> Contact.call(p);

        System.out.println("\n=== Call Adults ===");
        list.stream().filter(adult).filter(p -> p.getSurname().contains("C")).forEach(call);

        System.out.println("\n=== Call Youth ===");
        list.stream().filter(youth).forEach(call);
    }
